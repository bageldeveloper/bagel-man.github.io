...
function getTimeBasedFavorite(type) {
  ...
  const seed = getTimeBasedSeed(type);
  const seedNumber = seed.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  const randomIndex = Math.floor(seededRandom(seedNumber) * favoriteGames.length);
  
  return favoriteGames[randomIndex];
}
...
function renderTimeBasedSections() {
  ...
  // Monthly pick - random favorite
  const monthlySection = document.getElementById('monthlySection');
  const monthlyGames = monthlySection.querySelector('.time-games');
  monthlyGames.innerHTML = '';
  const monthlyPick = getTimeBasedFavorite('monthly');
  if (monthlyPick) monthlyGames.appendChild(createTimeBasedCard(monthlyPick));

  // Yearly pick - random favorite
  ...
  const yearlyPick = getTimeBasedFavorite('yearly');
  if (yearlyPick) yearlyGames.appendChild(createTimeBasedCard(yearlyPick));
}
